...
const en: Dict = {
  ...
  'giant.tok.caseNote': 'Case {c} / {n} · a new case every loop — same word, different context, different fate',
  'giant.tok.case0.word': 'apple',
  'giant.tok.case0.sentence': 'I |ate *1,2|an *1|[apple]| this morning*3| — |sweet*3|.',
  'giant.tok.case0.s0': 'Just the embedding for “apple” — identical in every sentence it appears in.',
  'giant.tok.case0.s1': 'Syntax absorbed: “ate” + “an” light up — it is a noun, the object of a verb.',
  'giant.tok.case0.s2': 'Semantics: something you can EAT — almost certainly the fruit.',
  'giant.tok.case0.s3': 'Context: “this morning” and “sweet” flow in — breakfast, definitely not Apple Inc.',
  'giant.tok.case0.s4': 'The whole sentence is fused into the vector — ready to predict the next token.',
  'giant.tok.case1.word': 'Amazon',
  'giant.tok.case1.sentence': '[Amazon]| shipped *1,2|my order *2|overnight*3|.',
  'giant.tok.case1.s0': 'The embedding for “Amazon” — right now it could be a company OR a rainforest.',
  'giant.tok.case1.s1': 'Syntax: it is the SUBJECT of “shipped”.',
  'giant.tok.case1.s2': 'Semantics: things that ship orders tend to be companies.',
  'giant.tok.case1.s3': 'Context: “overnight” seals it — the retailer, not the river.',
  'giant.tok.case1.s4': 'Note the next case starts from the SAME vector — and lands somewhere else entirely.',
  'giant.tok.case2.word': 'Amazon',
  'giant.tok.case2.sentence': 'We |got lost *1,2|in the |[Amazon]| rainforest*3| last year|.',
  'giant.tok.case2.s0': 'The very same “Amazon” embedding as the previous case.',
  'giant.tok.case2.s1': 'Syntax: it sits inside “in the …” — a place.',
  ...
  'arch.agent': 'AI Agent',
  'arch.agentFull': 'A complete AI agent — memory, routing, sub-agents, tools',
  'class.agent.0': 'Fix code + test',
  'class.agent.1': 'Recall preference',
  'class.agent.2': 'Deadline + reminder',
  'class.agent.3': 'Weekly summary',
  'layer.agentDialog': 'Dialogue',
  'layer.agentPlanner': 'Planner (router)',
  'layer.agentRetrieve': 'Retrieval agent',
  'layer.agentCode': 'Coding agent',
  'layer.agentRun': 'Execution agent',
  'layer.agentAnswer': 'Reply',
  'layer.agentSummarize': 'Memory summarizer',
  'layer.agentMemory': 'Long-term vector memory',
  'agent.userTag': 'USER',
  'agent.botTag': 'AGENT',
  'agent.dialogSub': 'request in · answer out',
  'agent.plannerSub': 'trained MLP router',
  'agent.codeSub': 'writes the patch',
  'agent.runSub': 'commands & tools',
  'agent.answerSub': 'assemble the reply',
  'agent.summarizeSub': 'compress → write back',
  'agent.memorySub': 'real embeddings · k-means clusters · PCA layout',
  'agent.routeTitle': 'Route probabilities (real softmax)',
  'agent.route0': 'retrieve',
  'agent.route1': 'code',
  'agent.route2': 'run',
  'agent.route3': 'summarize',
  'agent.termTitle': 'plan / code / terminal',
  'explain.agentDialog.what': 'The user-facing module: takes the request, and at the end of the loop returns the assembled answer.',
  'explain.agentDialog.why': 'Everything an agent does starts and ends here — the rest of the machinery exists to make this one reply correct.',
  'explain.agentDialog.simple': 'The front desk of the whole operation.',
  'explain.agentPlanner.what': 'A REAL trained MLP (the 13th net on this site) reads the request embedding and outputs softmax probabilities over sub-agents: retrieve / code / run / summarize.',
  'explain.agentPlanner.why': 'Multi-agent systems live or die by routing: sending the task to the wrong specialist wastes every downstream step. Production agents use an LLM for this; the principle — classify, then delegate — is identical.',
  'explain.agentPlanner.simple': 'A dispatcher deciding which specialist takes the call.',
  'explain.agentMemory.what': 'Long-term memories embedded as vectors (char n-gram hashing here), clustered by k-means into themes, laid out by PCA. Retrieval = cosine similarity between the request vector and every memory — the top-3 light up with their true scores.',
  'explain.agentMemory.why': 'An agent without memory repeats questions and forgets preferences. Vector search is how production agents (RAG) find relevant context in millions of records — same math, bigger embeddings.',
  'explain.agentMemory.simple': 'A library where books arrange themselves by topic, and the librarian pulls the three closest to your question.',
  'explain.agentCode.what': 'The coding sub-agent: takes the retrieved context and produces a patch (here: flipping the sort comparator).',
  'explain.agentCode.why': 'Specialised sub-agents beat one generalist: the coder only thinks about code, and its output goes straight to the executor for verification.',
  'explain.agentCode.simple': 'The engineer of the team.',
  'explain.agentRun.what': 'The execution sub-agent: runs commands and tools (tests, reminders, deploys) and captures their output.',
  'explain.agentRun.why': 'Tool use is what turns a language model into an agent — it stops describing actions and starts performing them, then reads the result back.',
  'explain.agentRun.simple': 'The hands of the team — and the eyes that read the terminal.',
  'explain.agentAnswer.what': 'Results from the sub-agents are assembled into one reply and returned to the dialogue module.',
  'explain.agentAnswer.why': 'The user asked one question, not three — synthesis is itself a skill, and bad assembly wastes good sub-agent work.',
  'explain.agentAnswer.simple': 'The editor who turns three memos into one clear sentence.',
  'explain.agentWriteback.what': 'The episode is compressed into one distilled memory, embedded, and stored in its nearest cluster (really computed: watch which cluster the new point joins).',
  'explain.agentWriteback.why': 'Write-back closes the loop: next time, this episode is retrievable context. Summarising before storing keeps the memory from silting up with raw transcripts.',
  'explain.agentWriteback.simple': 'Writing one line in the diary before closing the book.',
  'overview.agent.intro': 'One page, the whole agent: dialogue takes a request; a trained router picks sub-agents (real softmax); the retrieval agent searches a long-term vector memory (real embeddings, k-means clusters, cosine top-3 with visible scores); coding and execution agents patch code and run commands; the answer returns to the user; and a summarizer compresses the episode and writes it back to the correct cluster — really computed. This is the loop inside Claude Code, AutoGPT and every production agent, at inspectable scale.',
  'overview.agent.problems': 'A bare LLM is stateless and hands-off: it forgets you and cannot act. The agent loop adds memory (vector store + retrieval), decomposition (planner + sub-agents) and tool use (terminal) — the three things that turn a chat model into a worker.',
  'overview.agent.domains': 'Coding agents (Claude Code, Copilot Workspace)\nPersonal assistants with long-term memory\nRAG knowledge systems\nComputer-use / browser agents\nMulti-agent research & simulation (Generative Agents)',
  'overview.agent.industries': 'Software — autonomous coding & review\nCustomer service — context-aware support\nOffice work — email / schedule / document agents\nOperations — incident-response runbooks\nResearch — literature & data agents',
  'agent.shard': 'Shard {c} · {n} memories',
...
const zh: Dict = {
  ...
  'giant.tok.caseNote': '案例 {c} / {n} · 每轮换一个——同一个词，不同上下文，不同结局',
  'giant.tok.case0.word': '苹果',
  'giant.tok.case0.sentence': '我|早上*3|吃了*1,2|一个*1|[苹果]|，|很甜*3|。',
  'giant.tok.case0.s0': '只是「苹果」的词嵌入——出现在哪句话里都一模一样。',
  'giant.tok.case0.s1': '语法进来了：「吃了」「一个」点亮——它是名词，是动词的宾语。',
  'giant.tok.case0.s2': '语义成形：能「吃」的东西——大概率是水果。',
  'giant.tok.case0.s3': '上下文汇入：「早上」「很甜」也亮了——这是早餐，确定不是苹果公司。',
  'giant.tok.case0.s4': '整句话已融合进这个向量——交给输出层预测下一个 token。',
  'giant.tok.case1.word': '苹果',
  'giant.tok.case1.sentence': '[苹果]|今天*3|发布了*1,2|新一代*2|手机*3|。',
  'giant.tok.case1.s0': '同一个词「苹果」——出发点和上一个案例一模一样。',
  'giant.tok.case1.s1': '语法：它是「发布了」的主语——会发布东西的，像个机构。',
  'giant.tok.case1.s2': '语义：「发布了新一代…」——越来越像一家公司。',
  'giant.tok.case1.s3': '上下文：「手机」一锤定音——是苹果公司，不是水果！',
  'giant.tok.case1.s4': '同一个词走到了完全不同的含义——这就是上下文的力量，也是几十层存在的理由。',
  'giant.tok.case2.word': '小米',
  'giant.tok.case2.sentence': '我|早饭*3|喝了*1,2|一碗*1,2|[小米]|粥*3|。',
  'giant.tok.case2.s0': '「小米」的词嵌入——此刻它既可能是谷物，也可能是手机公司。',
  'giant.tok.case2.s1': '语法：「喝了」「一碗」——量词暴露了它能装进碗里。',
  ...
  'overview.mamba.industries':
    'AI 基础设施——更便宜的长上下文推理\n生物信息——基因组级模型\n语音技术——流式识别\n端侧 AI——固定内存占用',
  'arch.agent': 'AI Agent',
  'arch.agentFull': '完整 AI Agent——记忆、路由、多子 agent、工具',
  'class.agent.0': '改代码+测试',
  'class.agent.1': '回忆偏好',
  'class.agent.2': '查询+提醒',
  'class.agent.3': '每周总结',
  'layer.agentDialog': '对话模块',
  'layer.agentPlanner': '规划器（路由）',
  'layer.agentRetrieve': '检索 agent',
  'layer.agentCode': '编码 agent',
  'layer.agentRun': '执行 agent',
  'layer.agentAnswer': '汇总回复',
  'layer.agentSummarize': '记忆汇总',
  'layer.agentMemory': '长期记忆向量集群',
  'agent.userTag': '用户',
  'agent.botTag': 'AGENT',
  'agent.dialogSub': '请求进 · 回答出',
  'agent.plannerSub': '真实训练的 MLP 路由器',
  'agent.codeSub': '产出代码补丁',
  'agent.runSub': '命令与工具',
  'agent.answerSub': '组装最终回复',
  'agent.summarizeSub': '压缩 → 写回记忆',
  'agent.memorySub': '真实嵌入 · k-means 聚类 · PCA 布局',
  'agent.routeTitle': '路由概率（真实 softmax）',
  'agent.route0': '检索',
  'agent.route1': '编码',
  'agent.route2': '执行',
  'agent.route3': '汇总',
  'agent.termTitle': '计划 / 代码 / 终端',
  'explain.agentDialog.what': '面向用户的模块：接收请求，并在循环结束时交回组装好的答案。',
  'explain.agentDialog.why': 'Agent 做的一切从这里开始、到这里结束——其余所有机器都是为了让这一条回复正确。',
  'explain.agentDialog.simple': '整个公司的前台。',
  'explain.agentPlanner.what': '一个「真实训练」的 MLP（本站第 13 个网络）读取请求向量，输出四个子 agent 的 softmax 概率：检索 / 编码 / 执行 / 汇总。',
  'explain.agentPlanner.why': '多 agent 系统成败在路由：任务派错专家，后面每一步都白费。生产系统用 LLM 做这件事——「先分类,再分派」的原理完全相同。',
  'explain.agentPlanner.simple': '决定这通电话转给哪位专家的调度员。',
  'explain.agentMemory.what': '长期记忆被嵌入为向量（这里用字符 n-gram 哈希），k-means 聚成主题簇，按 PCA 摆放。检索 = 请求向量与每条记忆的余弦相似度——top-3 点亮并标出真实分数。',
  'explain.agentMemory.why': '没有记忆的 agent 会重复提问、忘记偏好。向量检索正是生产 agent（RAG）在百万条记录里找相关上下文的方法——数学相同,只是嵌入更大。',
  'explain.agentMemory.simple': '一座书会按主题自动归位的图书馆,管理员抽出离你的问题最近的三本。',
  'explain.agentCode.what': '编码子 agent：拿着检索到的上下文产出补丁（这里是把排序比较符改成降序）。',
  'explain.agentCode.why': '专职子 agent 胜过全能选手：编码者只想代码的事,产出直接交给执行者验证。',
  'explain.agentCode.simple': '团队里的工程师。',
  'explain.agentRun.what': '执行子 agent：运行命令与工具（测试、提醒、部署）,并截获输出。',
  'explain.agentRun.why': '工具调用是语言模型变成 agent 的分水岭——不再「描述动作」,而是「做动作」,然后读回结果。',
  'explain.agentRun.simple': '团队的双手——以及盯着终端的眼睛。',
  'explain.agentAnswer.what': '各子 agent 的结果被组装成一条回复,交回对话模块。',
  'explain.agentAnswer.why': '用户问的是一个问题,不是三个——「综合」本身是能力,组装得差会浪费子 agent 的好工作。',
  'explain.agentAnswer.simple': '把三份备忘录写成一句话的主编。',
  'explain.agentWriteback.what': '这一轮经历被压缩成一条精炼记忆,重新嵌入,存进最近的簇（真实计算：看新点加入了哪个簇）。',
  'explain.agentWriteback.why': '写回让循环闭合：下次这段经历就是可检索的上下文。先总结再存储,记忆库才不会被原始记录淤塞。',
  'explain.agentWriteback.simple': '合上书之前,在日记里写下一行。',
  'overview.agent.intro': '一页看完整个 Agent：对话模块接收请求；训练好的路由器挑选子 agent（真实 softmax）；检索 agent 在长期记忆向量集群中搜索（真实嵌入、k-means 聚类、余弦 top-3 且分数可见）；编码与执行 agent 改代码、跑命令；答案交回用户；汇总器把这轮经历压缩后写回正确的簇——同样是真实计算。这就是 Claude Code、AutoGPT 等一切生产 agent 内部的循环,缩到可以逐步检视的尺度。',
  'overview.agent.problems': '裸的 LLM 无状态也无行动力：记不住你,也做不了事。Agent 循环补上三样东西——记忆（向量库+检索）、分解（规划器+子 agent）、工具（终端）——聊天模型由此变成干活的。',
  'overview.agent.domains': '编码 agent（Claude Code、Copilot Workspace）\n带长期记忆的个人助理\nRAG 知识系统\n电脑操作 / 浏览器 agent\n多 agent 研究与模拟（Generative Agents）',
  'overview.agent.industries': '软件——自主编码与评审\n客服——带上下文的支持\n办公——邮件 / 日程 / 文档 agent\n运维——事故响应手册\n科研——文献与数据 agent',
  'agent.shard': '分片 {c} · {n} 条记忆',
  'overview.title': '模型简介',
...
const ja: Dict = {
  ...
  'giant.tok.caseNote': 'ケース {c} / {n} · ループごとに交代——同じ単語、違う文脈、違う結末',
  'giant.tok.case0.word': 'リンゴ',
  'giant.tok.case0.sentence': '今朝*3|、|[リンゴ]|を|一つ*1|食べた*1,2|。|甘かった*3|。',
  'giant.tok.case0.s0': '「リンゴ」の埋め込みベクトル——どの文に現れても同じ。',
  'giant.tok.case0.s1': '構文を吸収：「食べた」「一つ」が点灯——数えられる名詞で、動詞の目的語。',
  'giant.tok.case0.s2': '意味の形成：食べられるもの——ほぼ果物。',
  'giant.tok.case0.s3': '文脈が流れ込む：「今朝」「甘かった」——朝食の話。会社ではない。',
  'giant.tok.case0.s4': '文全体がこのベクトルに融合——出力層へ渡して次のトークンを予測。',
  'giant.tok.case1.word': 'アマゾン',
  'giant.tok.case1.sentence': '[アマゾン]|で|本を*2|注文した*1,2|。|翌日*3|届いた*3|。',
  'giant.tok.case1.s0': '「アマゾン」の埋め込み——今はまだ EC サイトか熱帯雨林か分からない。',
  'giant.tok.case1.s1': '構文：「注文した」場所を表している。',
  'giant.tok.case1.s2': '意味：本を注文できる——通販サイトらしい。',
  'giant.tok.case1.s3': '文脈：「翌日届いた」で確定——川ではなく会社。',
  'giant.tok.case1.s4': '次のケースは全く同じベクトルから出発して——別の意味に到達する。',
  'giant.tok.case2.word': 'アマゾン',
  'giant.tok.case2.sentence': '去年*3|、|[アマゾン]|の|熱帯雨林で*2,3|道に迷った*1,2|。',
  'giant.tok.case2.s0': '前のケースと全く同じ「アマゾン」の埋め込み。',
  'giant.tok.case2.s1': '構文：「〜の…で」——場所を指す。',
  ...
  'arch.agent': 'AI Agent',
  'arch.agentFull': '完全な AI エージェント——記憶・ルーティング・サブエージェント・ツール',
  'class.agent.0': 'コード修正+テスト',
  'class.agent.1': '好みを想起',
  'class.agent.2': '照会+リマインダー',
  'class.agent.3': '週次まとめ',
  'layer.agentDialog': '対話モジュール',
  'layer.agentPlanner': 'プランナー（ルーター）',
  'layer.agentRetrieve': '検索エージェント',
  'layer.agentCode': 'コーディングエージェント',
  'layer.agentRun': '実行エージェント',
  'layer.agentAnswer': '回答の組み立て',
  'layer.agentSummarize': '記憶要約',
  'layer.agentMemory': '長期記憶ベクトルクラスタ',
  'agent.userTag': 'ユーザー',
  'agent.botTag': 'AGENT',
  'agent.dialogSub': 'リクエスト入 · 回答出',
  'agent.plannerSub': '実際に学習した MLP ルーター',
  'agent.codeSub': 'パッチを作成',
  'agent.runSub': 'コマンドとツール',
  'agent.answerSub': '最終回答を組み立て',
  'agent.summarizeSub': '圧縮 → 書き戻し',
  'agent.memorySub': '実埋め込み · k-means クラスタ · PCA 配置',
  'agent.routeTitle': 'ルート確率（本物の softmax）',
  'agent.route0': '検索',
  'agent.route1': 'コード',
  'agent.route2': '実行',
  'agent.route3': '要約',
  'agent.termTitle': '計画 / コード / ターミナル',
  'explain.agentDialog.what': 'ユーザーに面するモジュール。リクエストを受け取り、ループの最後に組み立てた回答を返す。',
  'explain.agentDialog.why': 'エージェントの全てはここから始まりここで終わる——残りの機構はこの一つの返答を正しくするためにある。',
  'explain.agentDialog.simple': '会社全体の受付。',
  'explain.agentPlanner.what': '「実際に学習した」MLP（本サイト 13 個目のネット）がリクエストの埋め込みを読み、サブエージェント（検索/コード/実行/要約）の softmax 確率を出す。',
  'explain.agentPlanner.why': 'マルチエージェントはルーティングが命：間違った専門家に回せば以降の全ステップが無駄。本番では LLM がこれを担うが「分類して委任」の原理は同じ。',
  'explain.agentPlanner.simple': 'どの専門家に電話を回すか決める交換手。',
  'explain.agentMemory.what': '長期記憶をベクトル化（ここでは文字 n-gram ハッシュ）、k-means でテーマ別クラスタに、PCA で配置。検索 = リクエストと各記憶のコサイン類似度——top-3 が実スコア付きで点灯。',
  'explain.agentMemory.why': '記憶のないエージェントは同じ質問を繰り返し好みを忘れる。ベクトル検索は本番エージェント（RAG）が数百万件から文脈を見つける方法そのもの——数学は同じ、埋め込みが大きいだけ。',
  'explain.agentMemory.simple': '本がテーマごとに自ら並ぶ図書館で、司書が質問に最も近い 3 冊を抜き出す。',
  'explain.agentCode.what': 'コーディング担当サブエージェント。検索した文脈をもとにパッチを作る（ここではソートの比較子を降順に）。',
  'explain.agentCode.why': '専任サブエージェントは万能選手に勝る：コーダーはコードだけを考え、出力は即実行者の検証に回る。',
  'explain.agentCode.simple': 'チームのエンジニア。',
  'explain.agentRun.what': '実行担当サブエージェント。コマンドとツール（テスト・リマインダー・デプロイ）を走らせ、出力を取り込む。',
  'explain.agentRun.why': 'ツール使用こそ言語モデルがエージェントになる分水嶺——動作を「述べる」のをやめ「行い」、結果を読み返す。',
  'explain.agentRun.simple': 'チームの手——そしてターミナルを見つめる目。',
  'explain.agentAnswer.what': 'サブエージェントの結果を一つの返答に組み立て、対話モジュールへ返す。',
  'explain.agentAnswer.why': 'ユーザーの質問は一つ、三つではない——「統合」自体が能力で、下手な組み立ては良い仕事を無駄にする。',
  'explain.agentAnswer.simple': '三つのメモを一文にまとめる編集長。',
  'explain.agentWriteback.what': '今回のエピソードを一件の蒸留された記憶に圧縮し、再埋め込みして最も近いクラスタへ保存（実計算：新しい点がどの簇に入るか見よ）。',
  'explain.agentWriteback.why': '書き戻しでループが閉じる：次回このエピソードは検索可能な文脈になる。要約してから保存するから、記憶が生ログで淤まない。',
  'explain.agentWriteback.simple': '本を閉じる前に日記へ一行書く。',
  'overview.agent.intro': '1 ページでエージェントの全体像：対話がリクエストを受け、学習済みルーターがサブエージェントを選び（本物の softmax）、検索エージェントが長期記憶ベクトルクラスタを探索（実埋め込み・k-means・コサイン top-3 のスコア可視）、コーディングと実行のエージェントがコードを直しコマンドを走らせ、回答がユーザーへ戻り、要約器がエピソードを圧縮して正しいクラスタへ書き戻す——これも実計算。Claude Code や AutoGPT の内部ループを、検分できるスケールで。',
  'overview.agent.problems': '素の LLM は無状態で手も出せない：あなたを忘れ、行動もできない。エージェントループは記憶（ベクトルストア+検索）・分解（プランナー+サブエージェント）・ツール（ターミナル）の 3 つを足す——チャットモデルが働き手になる。',
  'overview.agent.domains': 'コーディングエージェント（Claude Code など）\n長期記憶つきパーソナルアシスタント\nRAG 知識システム\nコンピュータ操作 / ブラウザエージェント\nマルチエージェント研究（Generative Agents）',
  'overview.agent.industries': 'ソフトウェア——自律的なコーディングとレビュー\nカスタマーサポート——文脈を保った対応\nオフィス業務——メール / 日程 / 文書エージェント\n運用——インシデント対応\n研究——文献・データエージェント',
  'agent.shard': 'シャード {c} · {n} 件の記憶',
